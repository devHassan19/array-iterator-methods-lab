...
sortedByBirthYear = inventors.sort((inventorsA, inventorsB) => {
  return inventorsA.year - inventorsB.year
})
...
inventorNamedAda = inventors.find((inventors) => {
...
commentById = comments.find((comments) => {
...
totalYearsLived = inventors.reduce(function (accumulator, inventor) {
  const yearLive = inventor.passed - inventor.year
  return (accumulator += yearLive)
}, 0)
// Check your work:
...
travelMethodCounts = travelMethods.reduce(function (acc, travel) {
  if (acc[travel]) {
    acc[travel] = acc[travel] + 1
  } else {
    acc[travel] = 1
  }
  return acc
